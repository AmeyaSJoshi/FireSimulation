...
## 2026-07-26 update 15 (Claude) -- block-scale retarget: 32 km/500 m -> 640 m/10 m, buildings/roads added

Product scope change to the interactive path (not the benchmark/regional path, which is untouched and re-verified identical): the model field is now 640 m at 10 m/cell -- ESA WorldCover's native resolution -- instead of 32 km at 500 m. Grid stays 64x64; only `FIRE_CELL_SIZE_KM` (0.5 -> 0.01) and `FIRE_MAX_PROPAGATION_HOURS` (72 -> 2) changed in `main.js`. `firePropagation.js`'s Dijkstra core, `surfaceSpread.js`, `fireEllipse.js`, `crownFire.js`, and the moisture models were not touched -- `cellSizeMeters`/`gridSize` were already pure inputs, confirmed by grep before editing.

At 10 m, a model cell *is* one WorldCover pixel, so the 4x4 sub-sample majority vote in `fetchFineLandCoverField` (sized for averaging across a 500 m cell) degenerated to a single centre sample -- there is nothing left to vote between. `enableSpotting` is now passed explicitly (`false`) end to end with a `console.assert`, rather than relying on the solver's own default, since a 640 m field is 8x narrower than `SPOTTING_MAX_DISTANCE_METERS` (5000 m).

Added `src/lib/urbanFootprints.js` (OpenStreetMap via Overpass -- buildings, roads, landuse/leisure/natural green space; probed once per session, cached by bbox, graceful fallback to WorldCover-only on failure) and `src/renderers/blockScene.js` (a local Three.js scene: ground plane with a fuel-color raster, extruded+merged building geometry, buffered road quads, `OrbitControls`, and a fire shader that reads the already-solved arrival-time field -- `firePropagation.js`'s `arrivalTimes`, sent once per run from the worker -- against a scrubbable `uTime` uniform; scrubbing the new timeline slider changes the shader instantly with zero worker/network traffic). Buildings and roads become non-burnable through the *existing* WorldCover "Built-up" (classCode 50) hard-barrier path in `landCoverToFuel.js`, not a new fuel-decision branch; OSM green space maps onto existing WorldCover vegetation classes the same way.

One real bug caught in verification, not anticipated in the plan: at block scale, `burnedAreaKm2`/`footprintAreaKm2`/etc. are almost always well under 1 km2 (one cell = 0.0001 km2), so the existing `Math.round(...) + "km2"` UI formatting rounded every legible result to "0 km2" even while the fire was visibly spreading. Added scale-aware formatters (m2/m/m-per-h below 0.1 of the km unit) across the live readout, run history, and ensemble-range text.

No accuracy claim is made at this scale anywhere in the UI; the "Model provenance" panel now states scope (640 m / 10 m cells), building/road source, and that spotting is off, explicitly. `validate:rothermel`, `validate:rothermel:rate`, `validate:waf`, and `validate:convergence` all pass with numbers identical to before this change.
